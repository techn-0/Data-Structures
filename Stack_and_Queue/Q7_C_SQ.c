...
{
	/* add your code here */
	// 새 스텍 s에 expression의 첫번쨰 원소가 열리는 괄호 종류라면 하나씩 푸쉬
	// 열리는 괄호가 아니라면 새 변수에 s의 팝을 저장한후 닫히는 괄호가 일치하는지 확인
	Stack s;
	s.ll.head = NULL;
	s.ll.size = 0;
	...
	while (*expression != '\0') // 문자열 끝까지 반복
	{
		item = *expression; // expression의 첫번째 문자
