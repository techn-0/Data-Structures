...

{
    ...
    // 현재 노드가 자식이 하나인지 확인
    int isOneChildNode = (node->left == NULL && node->right != NULL) || (node->left != NULL && node->right == NULL);

    // 자식이 하나만 있는 노드가 있으면 +1
    return L + R + (isOneChildNode ? 1 : 0);
}
